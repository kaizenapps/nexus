...
export default function Intelligence() {
    ...
    // Some mock data for "Insights Grid" until we implement real metrics for them
    // Growth Velocity could be calculated if we had timestamps on nodes
    // Resilience Score could be based on graph density/connectivity
    const growthVelocity = "+12%";
    const resilienceScore = "8.4/10";
    const aiSuggestions = strategicMove ? 1 : 0;
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                    ...
                    <p className="text-xl font-bold text-white truncate">{metrics.topIndustries[0]?.[0] || 'N/A'}</p>
                    <p className="text-xs text-gray-500">{metrics.topIndustries[0]?.[1] || 0} companies</p>
                </div>
            </div>
                ...
                <div className="bg-gradient-to-br from-purple-900/40 to-blue-900/40 border border-white/10 rounded-xl p-6 backdrop-blur-sm relative overflow-hidden group hover:border-purple-500/50 transition-all duration-300">
                    ...
                    {strategicMove ? (
                        <div className="space-y-4 relative z-10">
                            ...
                            <button
                                onClick={() => navigate(`/graph?search=${encodeURIComponent(strategicMove.name)}`)}
                         ...
                         <div className="space-y-4 relative z-10">
                            <p className="text-gray-300">No strategic moves identified yet.</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                    ...
                    <p className="text-2xl font-bold text-white">{growthVelocity}</p>
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                    ...
                    <p className="text-2xl font-bold text-white">{resilienceScore}</p>
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                    ...
                    <p className="text-2xl font-bold text-white">{aiSuggestions}</p>
